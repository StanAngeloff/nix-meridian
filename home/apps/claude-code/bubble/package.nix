...
# Modules may also declare `grants` in module.nix ({ name, description }): each becomes an accepted `--with-<name>` launcher flag, consumed by the skeleton into the bubble_grants array that hooks branch on, and documented after Claude Code's own --help output.
