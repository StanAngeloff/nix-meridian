...
# pnpm hardlinks packages between its content-addressable store and node_modules, and hardlinks cannot cross bubblewrap mount boundaries (they fail with EXDEV), so the store must sit on the project's own mount. The bubble binds only the project and never a shared parent (which would expose sibling repositories), so the store lives inside the project at <project>/node_modules/.pnpm-store. Placing it under node_modules avoids file-watcher loops: pnpm's store/v11/projects/<hash> is a symlink back to the project root, and a store at the project root creates infinite recursion for any watcher that follows symlinks (vite, chokidar). Every tool already excludes node_modules from watching. That duplicates packages across repositories, the deliberate cost of keeping the boundary tight.
# The redirect is pnpm_config_store_dir pointing at the default ~/.local/share/pnpm/store path, which is a symlink into the project-local store. pnpm records the path it is handed verbatim rather than its resolved target, so .modules.yaml carries the default store path and an unbubbled host `pnpm install` of the same project agrees without any host-side configuration.
...
pnpm_prepare() {
	...
	if [[ -n "${bubble_grants[pnpm]:-}" || -f "$project_path/package.json" ]]; then
		pnpm_active=1
		mkdir -p "$project_path/node_modules/.pnpm-store"
	fi
}
...
pnpm_mount() {
	if [[ -n "${pnpm_active:-}" ]]; then
		bwrap_args+=(
			...
			--symlink "$project_path/node_modules/.pnpm-store" "$home_path/.local/share/pnpm/store"
		)
	fi
}
...
pnpm_environment() {
	if [[ -n "${pnpm_active:-}" ]]; then
		bwrap_args+=(--setenv pnpm_config_store_dir "$home_path/.local/share/pnpm/store")
	fi
}
